...
import { Search, Calendar, Building, RefreshCw, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function SiteSoilExpensesManagement({
  ...
  const [soilExpensesList, setSoilExpensesList] = useState<any[]>([]);
  ...
  // 백엔드 실제 DB 흙값 정산 데이터 로드
  const fetchSoilSettlementData = useCallback(async () => {
    try {
      ...
      const url = `${baseUrl}/api/dispatch/settlements/site-soil-expenses${
      ...
      if (res.ok) {
        ...
        setSummaryData(data.summary || { totalExpense: 0, totalIncome: 0, netBalance: 0, totalCount: 0 });
        setSoilExpensesList(data.soilExpenses || []);
      } else {
        console.warn("흙값 정산 API 응답 상태:", res.status);
        setSummaryData({ totalExpense: 0, totalIncome: 0, netBalance: 0, totalCount: 0 });
        setSoilExpensesList([]);
      }
    } catch (e) {
      console.warn("흙값 정산 조회 알림:", e);
      setSummaryData({ totalExpense: 0, totalIncome: 0, netBalance: 0, totalCount: 0 });
      setSoilExpensesList([]);
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/dispatch/settlements/site-soil-expenses/status`, {
        ...
        body: JSON.stringify({
          ...
          dispute_type: extraData?.dispute_type || "SITE",
  ...
  // 필터링 및 작업일자 기준 최신순 정렬 보장
  const filteredData = soilExpensesList
    .filter((item) => {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex justify-between items-center border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={fetchSoilSettlementData}
            disabled={isLoading}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-2">
          ...
          <p className="text-[10px] text-slate-500 font-semibold">
            조회 건수: <strong className="text-slate-800">{filteredData.length}건</strong>
          </p>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-3">
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto flex-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="현장명, 사토장, 토사 검색..."
              className="pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:border-blue-500 w-48"
            />
          </div>
        </div>
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
          ...
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
              payerTypeFilter === "SITE_PAYS" ? "bg-rose-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-900"
            }`}
          >
            현장지급
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
              payerTypeFilter === "SITE_RECEIVES" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-900"
            }`}
          >
            하차지지급
          </button>
      ...
      <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xl space-y-4">
        <div className="flex justify-between items-center border-b border-slate-100 pb-3">
          <h4 className="font-extrabold text-sm text-slate-900">토사 정산 대장 (작업일자 최신순)</h4>
          <span className="text-xs text-slate-500 font-semibold">총 {filteredData.length}건</span>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[950px]">
            <thead>
              <tr className="bg-slate-50 border-y border-slate-200 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider whitespace-nowrap">
                ...
                <th className="py-3 px-3 min-w-[140px]">현장명</th>
                  ...
                  <td className="py-3.5 px-3 text-center whitespace-nowrap">
                    ...
                    {row.status === "이의제기" && row.disputeType && (
                      <span className="block text-[9px] text-rose-500 font-bold mt-0.5 whitespace-nowrap">
                        [{row.disputeType === "SITE" ? "현장 제기" : "하차지 제기"}]
                      </span>
                    )}
                  </td>
                  <td className="py-3.5 px-3 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-1.5 flex-wrap">
                      ...
                      {row.status === "정산 검토" && (
                        <>
                          {/* 현장이 돈을 지급하는 경우(SITE_PAYS): 검토 후 [송금 처리] 또는 문제 시 [이의제기/보류] */}
                          {row.payerType === "SITE_PAYS" ? (
                            <>
                              <button
                                type="button"
                            ...
                            /* 현장이 돈을 받는 경우(SITE_RECEIVES): 하차지가 먼저 송금하기 전이므로 송금 대기 (이의제기 불가) */
                            <span className="text-[11px] text-amber-600 font-bold">하차지 송금 대기</span>
                          ) : (
                            /* 무상 처리 */
                            <button
                              type="button"
                      ...
                      {row.status === "송금 완료" && (
                        <>
                          {/* 현장이 돈을 받는 경우(SITE_RECEIVES): 하차지가 송금했으므로 입금 확인 후 [입금 확인] 또는 금액 부족 시 [이의제기] */}
                          {row.payerType === "SITE_RECEIVES" ? (
                            <>
                              <button
                                type="button"
                            ...
                            /* 현장이 돈을 보낸 경우(SITE_PAYS): 상대(하차지)가 수령 확인할 때까지 대기 */
                            <span className="text-[11px] text-indigo-600 font-bold">하차지 수령 확인 대기</span>
                          )}
                        </>
                      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 space-y-4 overflow-y-auto text-xs">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 grid grid-cols-2 gap-2">
                ...
                <div>
                  <span className="text-slate-400 block text-[10px] font-bold">거래 구분 및 계약액</span>
              ...
              {disputeModalItem.status === "이의제기" ? (
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="font-extrabold text-[11px]">
                        ...
                        <strong className="underline">
                          {disputeModalItem.disputeType === "SITE" ? "상차 현장 관리자" : "하차 사토장 관리자"}
                        </strong>
                      </span>
                  ...
                  <p className="text-[11px] text-slate-500">
                    상호 유선 협의 및 수량/단가 확인 후 협의가 완료되면 [보류 해제 및 재검토]를 진행하거나 정산을 완료 처리할 수 있습니다.
                  </p>
                </div>
              ) : (
                /* 신규 이의제기 작성 폼 */
                <div className="space-y-3">
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[11px] leading-relaxed">
                    ℹ️ 실제 반출입 수량 불일치, 토질 불량(뻘흙/암버럭 혼입), 단가 이견 등으로 정산 금액에 차이가 있을 때 이의를 제기하고 <strong>정산 진행을 보류</strong>합니다.
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="예: 25대 계약 중 3대 분량이 뻘흙 혼입으로 반출 거부되어 실제 22대 기준으로 정산 차액 135,000원 감액 요청함"
