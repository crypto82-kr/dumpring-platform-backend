...
class OwnerRegister(BaseModel):
    ...
    # 필수 제출 서류 파일명/경로 (가입 시 서류 미첨부 시 기본값 허용)
    biz_license_file: Optional[str] = Field("temp", description="사업자등록증 파일명")
    machinery_reg_file: Optional[str] = Field("temp", description="건설기계 등록증·검사증 파일명")
    insurance_file: Optional[str] = Field("temp", description="보험가입증 파일명")
...
class SiteManagerRegister(BaseModel):
    ...
    # 필수 제출 서류 파일명/경로 (가입 시 서류 미첨부 시 기본값 허용)
    dust_report_file: Optional[str] = Field("temp", description="비산먼지 배출신고서 파일명")
    construction_contract_file: Optional[str] = Field("temp", description="공사 계약서 파일명")
...
class DropOffRegister(BaseModel):
    ...
    # 필수 제출 서류 파일명/경로 (가입 시 서류 미첨부 시 기본값 허용)
    development_permit_file: Optional[str] = Field("temp", description="개발행위 허가증 파일명")
    land_use_agreement_file: Optional[str] = Field("temp", description="토지 사용 승낙서 / 토지 대장 파일명")
